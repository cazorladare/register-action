...
#   Secrets:
#     - YYDSMAIL_API_KEY   (必填)
#     - YYDSMAIL_API_BASE  (可选)

DEFAULT_YYDSMAIL_API_BASE = "https://maliapi.215.im"
...
YYDSMAIL_API_BASE = _env("YYDSMAIL_API_BASE", DEFAULT_YYDSMAIL_API_BASE)
YYDSMAIL_API_KEY = _env("YYDSMAIL_API_KEY")


def validate_mail_config() -> None:
    missing = []
    if not YYDSMAIL_API_BASE:
        missing.append("YYDSMAIL_API_BASE")
    if not YYDSMAIL_API_KEY:
        missing.append("YYDSMAIL_API_KEY")

    if missing:
        raise RuntimeError(
            "缺少 YYDS Mail GitHub 配置: "
            + ", ".join(missing)
...
def create_temp_email(proxies: Any = None) -> tuple[str, str, str]:
    validate_mail_config()

    session = _create_mail_session(proxies)
    resp = session.post(
        f"{YYDSMAIL_API_BASE.rstrip('/')}/v1/accounts",
        headers={"X-API-Key": YYDSMAIL_API_KEY},
        timeout=15,
    )

    if resp.status_code not in (200, 201):
        raise RuntimeError(f"创建邮箱失败，状态码: {resp.status_code}，响应: {resp.text[:200]}")

    data = resp.json()
    if not data.get("success"):
        raise RuntimeError(f"创建邮箱失败: {data.get('error', '未知错误')}")

    result = data.get("data", {}) if isinstance(data, dict) else {}
    email = str(result.get("address") or "").strip()
    mail_token = str(result.get("token") or "").strip()
    inbox_id = str(result.get("id") or "").strip()
    if not email or not mail_token or not inbox_id:
        raise RuntimeError(f"创建邮箱返回数据不完整: {data}")
    return email, mail_token, inbox_id


def delete_temp_email(mail_token: str, inbox_id: str, proxies: Any = None) -> bool:
    if not mail_token or not inbox_id:
        return False

    try:
        session = _create_mail_session(proxies)
        resp = session.delete(
            f"{YYDSMAIL_API_BASE.rstrip('/')}/v1/accounts/{inbox_id}",
...
def _fetch_emails_yydsmail(mail_token: str, email_addr: str, proxies: Any = None) -> list[dict[str, Any]]:
    try:
        session = _create_mail_session(proxies)
        resp = session.get(
            f"{YYDSMAIL_API_BASE.rstrip('/')}/v1/messages",
        ...
        print(f"[YYDS Mail] 获取邮件列表错误: {e}")
        return []


def _fetch_email_detail_yydsmail(mail_token: str, msg_id: str, email_addr: str, proxies: Any = None) -> Optional[Dict[str, Any]]:
    try:
        session = _create_mail_session(proxies)
        resp = session.get(
            f"{YYDSMAIL_API_BASE.rstrip('/')}/v1/messages/{msg_id}",
            params={"address": email_addr},
            headers={"Authorization": f"Bearer {mail_token}"},
            timeout=15,
        )
        ...
        if isinstance(detail, dict):
            ...
            if isinstance(html, list):
                detail["html"] = html[0] if html else ""
            return detail
    except Exception as e:
        print(f"[YYDS Mail] 获取邮件详情错误: {e}")
    return None


def _extract_verification_code(content: str, subject: str = "") -> str:
    combined = f"{subject} {content}" if subject else content
    if not combined:
        return ""
...
def snapshot_message_ids(mail_token: str, email_addr: str, proxies: Any = None) -> set[str]:
    ids: set[str] = set()
    for msg in _fetch_emails_yydsmail(mail_token, email_addr, proxies) or []:
...
def get_oai_code(token: str, email_addr: str, proxies: Any = None, skip_ids: set = None) -> str:
    ...
    for _ in range(40):
        ...
        try:
            messages = _fetch_emails_yydsmail(token, email_addr, proxies) or []
            for msg in messages:
                ...
                detail = _fetch_email_detail_yydsmail(token, msg_id, email_addr, proxies)
                seen_ids.add(msg_id)
                if not detail:
                    continue
                ...
                body = str(detail.get("text") or html_content or "")
                code = _extract_verification_code(body, subject)
                if code:
                    print(f" 抓到啦! 验证码: {code}")
                    return code
        except Exception:
...
def run(proxy: Optional[str]) -> Optional[str]:
    ...
    try:
        email, mail_token, inbox_id = create_temp_email(proxies)
    except Exception as e:
        print(f"[Error] 获取 YYDS Mail 临时邮箱失败: {e}")
        return None

    print(f"[*] 成功获取 YYDS Mail 临时邮箱: {email}")
    ...
    try:
        ...
        _pre_login_mail_ids = snapshot_message_ids(mail_token, email, proxies)

        code = get_oai_code(mail_token, email, proxies)
        if not code:
            return None
    ...
    try:
        ...
        if page_type == "email_otp_verification" or "email-verification" in continue_url or "email-otp" in continue_url:
            ...
            login_otp = get_oai_code(mail_token, email, proxies, skip_ids=_pre_login_mail_ids)
            if not login_otp:
        ...
        delete_temp_email(mail_token, inbox_id, proxies)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="OpenAI 自动注册脚本 (YYDS Mail 版本)")
    parser.add_argument(
    ...
    attempt_count = 0
    success_count = 0
    print("[Info] Yasal's Seamless OpenAI Auto-Registrar Started for ZJH (YYDS Mail Edition)")
